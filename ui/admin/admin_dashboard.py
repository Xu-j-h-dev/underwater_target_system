...
from services import auth_service, model_manager, db_service, feedback_service
import config
...
class AdminDashboard(QMainWindow):
    ...
    def init_ui(self):
        ...
        # 反馈管理选项卡
        self.feedback_tab = self.create_feedback_management_tab()
        self.tab_widget.addTab(self.feedback_tab, '🐛 反馈管理')
    ...
    def create_feedback_management_tab(self):
        """创建反馈管理选项卡"""
        from ui.admin.feedback_management import FeedbackManagementDialog
        ...
        refresh_btn = QPushButton('🔄 刷新')
        refresh_btn.clicked.connect(self.open_feedback_management)
        toolbar.addWidget(refresh_btn)
        ...
        # 提示信息
        hint_label = QLabel('点击上方"刷新"按钮打开反馈管理界面')
        hint_label.setStyleSheet('color: #7f8c8d; font-size: 14px; padding: 20px;')
    ...
    def open_feedback_management(self):
        """打开反馈管理界面"""
        from ui.admin.feedback_management import FeedbackManagementDialog
        dialog = FeedbackManagementDialog(self)
        dialog.exec()

    def load_feedbacks(self):
        """加载反馈数据（初始化时调用）"""
        # 这个方法主要用于初始化时调用，实际加载在反馈管理界面中完成
        pass
